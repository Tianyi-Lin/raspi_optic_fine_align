import sys
import time
from pathlib import Path


_BUS_SERVO_DIR = Path(__file__).resolve().parent / "bus_servo"
if str(_BUS_SERVO_DIR) not in sys.path:
    sys.path.insert(0, str(_BUS_SERVO_DIR))

from driver import BusServoDriver
from transport import SerialTransport


class BusServo:
    def __init__(self, port="/dev/ttyAMA0", baudrate=9600, servo_num=2, servo_ids=None, moving_time=50):
        self.servo_num = servo_num
        self.servo_ids = servo_ids if servo_ids is not None else [1, 2]
        self.moving_time = moving_time

        self.transport = SerialTransport(port=port, baudrate=baudrate, timeout=1.0)
        self.driver = BusServoDriver(self.transport)

        self.servo_angles_setting = [[sid, 500] for sid in self.servo_ids]
        self.servo_angles_reading = [[sid, 500] for sid in self.servo_ids]

        self.pan_min_angle = -12
        self.pan_max_angle = 12
        self.pan_min_angle_index = 450
        self.pan_max_angle_index = 550

        self.tilt_min_angle = -12
        self.tilt_max_angle = 12
        self.tilt_min_angle_index = 450
        self.tilt_max_angle_index = 550

        self.pan_bound_flag = False
        self.tilt_bound_flag = False
        self.angle_equal_threshold = 2

        self.reset()

    def set_angle(self, servo_id, angle):
        if servo_id not in self.servo_ids:
            print("servo_id invalid!")
            return None

        if servo_id == 1:
            self.pan_bound_flag = False
            if angle > self.pan_max_angle:
                angle = self.pan_max_angle
                self.pan_bound_flag = True
                print("pan angle too big")
            elif angle < self.pan_min_angle:
                angle = self.pan_min_angle
                self.pan_bound_flag = True
                print("pan angle too small")
            angle_index = self.map(
                angle,
                self.pan_min_angle,
                self.pan_max_angle,
                self.pan_min_angle_index,
                self.pan_max_angle_index,
            )
            self._update_setting(servo_id, angle_index)
            return self.pan_bound_flag

        if servo_id == 2:
            self.tilt_bound_flag = False
            if angle > self.tilt_max_angle:
                angle = self.tilt_max_angle
                self.tilt_bound_flag = True
                print("tilt angle too big")
            elif angle < self.tilt_min_angle:
                angle = self.tilt_min_angle
                self.tilt_bound_flag = True
                print("tilt angle too small")
            angle_index = self.map(
                angle,
                self.tilt_min_angle,
                self.tilt_max_angle,
                self.tilt_min_angle_index,
                self.tilt_max_angle_index,
            )
            self._update_setting(servo_id, angle_index)
            return self.tilt_bound_flag

        angle_index = max(0, min(1000, int(round(angle))))
        self._update_setting(servo_id, angle_index)
        return False

    def move_angle(self):
        for servo_id, angle_index in self.servo_angles_setting:
            self.driver.move_time_write(servo_id, angle_index, self.moving_time)
        time.sleep(self.moving_time / 1000)

    def are_lists_soft_equal(self, list1, list2):
        if len(list1) != len(list2):
            return False
        for i in range(len(list1)):
            if abs(list1[i][1] - list2[i][1]) > self.angle_equal_threshold:
                return False
        return True

    def read_servos_angle(self):
        for i, servo_id in enumerate(self.servo_ids):
            angle = self.driver.read_pos(servo_id)
            self.servo_angles_reading[i] = [servo_id, angle]

    def map(self, x, in_min, in_max, out_min, out_max):
        return round((x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min)

    def reset(self):
        if 1 in self.servo_ids:
            self.set_angle(1, 0)
        if 2 in self.servo_ids:
            self.set_angle(2, 0)
        self.move_angle()
        print("reset complete!")

    def cleanup(self):
        self.driver.close()

    def _update_setting(self, servo_id, angle_index):
        for i in range(len(self.servo_angles_setting)):
            if self.servo_angles_setting[i][0] == servo_id:
                self.servo_angles_setting[i][1] = angle_index
                return
        self.servo_angles_setting.append([servo_id, angle_index])


if __name__ == "__main__":
    bus_servo = BusServo()
    while True:
        servo_id = int(input("Enter servo id: "))
        angle = float(input("Enter angle, pan id = 1 (-12 to 12) ; tilt = 2 (-12 to 12): "))
        bus_servo.set_angle(servo_id, angle)
        bus_servo.move_angle()
        if input("按q退出") == "q":
            break
    bus_servo.cleanup()
